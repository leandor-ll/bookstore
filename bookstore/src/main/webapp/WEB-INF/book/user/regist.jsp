...
<script type="text/javascript">
	$(function(){		
		...
		$("#uname").blur(function(){
			...
			//发送ajax请求给后台
			$.post("${pageContext.request.contextPath}/registUnameCheck",
					{"uname":uname},
					function(data){
				if(data.result && uPattern.test(uname)){
					//用户名可用
					$("#spanUsername").text("用户名可用").css("color","lightgreen");
				}else{
					if(data.result){
						//用户名不可用
						$("#spanUsername").text("用户名只能输入4到8位字母，数字，下划线，减号").css("color","red");
						$("#btnReg").attr("disabled",true);
					}else{
						//用户名不可用
						$("#spanUsername").text("用户名已存在").css("color","red");
						$("#btnReg").attr("disabled",true);
					}
				}
			},"json");
					...
					<table border="0"
						...
						<tr height="50">
							...
							<td>
								...
								<input type="text" title="可输入4到8位（字母，数字，下划线，减号）" class="l_user" name="uname" id="uname"/>
							</td>
						</tr>
						...
						<tr height="30">
							<td align="right"><font color="#ff4e00">*</font>身份信息 &nbsp;</td>
							<td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
								<input type="radio" value="1"  name="udentity" id="udentity" checked="checked"/>用户
								&nbsp;&nbsp;&nbsp;&nbsp;
							</td>
						</tr>
						...
						<tr height="70">
							<td>&nbsp;</td>
							<td><input type="submit" value="立即注册" class="log_btn" id="btnReg" disabled="disabled"/></td>
						</tr>
					</table>
				</form>
			</div>
